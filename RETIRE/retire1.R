...
plot_nav <- function(nav) {
  ...
  matplot(nav,
          ...
          main = 'Projected Value of Initial Investment')
  ...
  plot(density(final.nav, from=0, to=max(final.nav)), las = 1, xlab = 'Final Amount',
       
       main = paste0('Distribution of Final Amount\n', 100 * p.alive[last.period], '% are still paying'))
  ...
  wellPanel(
    ...
    fluidRow(
      ...
      column(6,
             ...
             sliderInput(paste0(prefix, "_", "n_obs"), "Years to retirement:", min = 0, max = 40, value = 20),
             
             sliderInput(paste0(prefix, "_", "start_capital"), "Initial investment ($):", min = 100000, max = 10000000, value = 2000000, step = 100000, pre = "$", sep = ","),
             
             sliderInput(paste0(prefix, "_", "annual_mean_return"), "Annual return (%):", min = 0.0, max = 30.0, value = 5.0, step = 0.5),
             
             sliderInput(paste0(prefix, "_", "annual_ret_std_dev"), "Annual volatility (%):", min = 0.0, max = 25.0, value = 7.0, step = 0.1)
             
      ),
      
      column(6,
             
             sliderInput(paste0(prefix, "_", "annual_inflation"), "Annual inflation (%):", min = 0, max = 20, value = 2.5, step = 0.1),
             
             sliderInput(paste0(prefix, "_", "annual_inf_std_dev"), "Annual inflation volatility (%):", min = 0.0, max = 5.0, value = 1.5, step = 0.05),
             
             sliderInput(paste0(prefix, "_", "monthly_withdrawals"), "Monthly withdrawal amount ($):", min = 1000, max = 100000, value = 10000, step = 1000, pre = "$", sep = ","),
...
ui <- fluidPage(theme="simplex.min.css",
                ...
                tags$h2("Retirement: overall value w/ random returns, inflation and withdrawals"),
                ...
                fluidRow(
                  ...
                  column(6, tags$h3("Scenario 1")),
                  
                  column(6, tags$h3("Scenario 2"))
